...
class _LivestockStep1ProjectBackgroundState
  ...
  bool _canProceed() {
    final purposeSelected = c.purposeBreeding || c.purposeMeat || c.purposeDairy;
  ...
  Widget _buildForm() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      CropField(label: 'FCA Name', hint: 'Enter FCA Name',
        initialValue: c.fcaName, onChanged: (v) => c.fcaName = v),
      const SizedBox(height: 20),

      CropDropdown(label: 'Region', hint: 'Choose Region',
        value: c.region, items: _regions,
      ...
      CropDropdown(label: 'Province', hint: 'Choose Province',
        value: c.province,
      ...
      CropDropdown(label: 'Municipality', hint: 'Choose Municipality',
      ...
      CropDropdown(label: 'Barangay', hint: 'Choose Barangay',
        value: c.barangay,
        items: c.municipality != null
            ? (_barangays[c.municipality!] ?? ['Barangay 1','Barangay 2','Barangay 3'])
            : [],
        enabled: c.municipality != null,
        onChanged: (v) => setState(() => c.barangay = v)),
      const SizedBox(height: 20),

      CropField(label: 'Project Title', hint: 'Based on BEDs submission',
        initialValue: c.projectTitle, onChanged: (v) => c.projectTitle = v),
      const SizedBox(height: 20),

      CropDropdown(label: 'Primary Intervention Provided',
        hint: 'Choose livestock raised',
        value: c.primaryIntervention, items: _primaryInterventions,
        onChanged: (v) => setState(() => c.primaryIntervention = v)),
      if (c.primaryIntervention == 'Others') ...[
        const SizedBox(height: 12),
        CropField(label: 'Please specify', hint: 'Enter primary intervention',
        ...
        return Padding(
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
            ...
            if (i > 0) ...[
              ...
              GestureDetector(
                onTap: () => setState(() => _supportInterventions.removeAt(i)),
      ...
      const SizedBox(height: 12),
      _buildCheckbox(label: 'Breeding', value: c.purposeBreeding,
          onChanged: (v) => setState(() => c.purposeBreeding = v ?? false)),
      const SizedBox(height: 10),
      _buildCheckbox(label: 'Meat / Fattener', value: c.purposeMeat,
          onChanged: (v) => setState(() => c.purposeMeat = v ?? false)),
      const SizedBox(height: 10),
      _buildCheckbox(label: 'Dairy', value: c.purposeDairy,
      ...
      child: Row(children: [
        AnimatedContainer(duration: const Duration(milliseconds: 150),
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: value ? DAColors.greenMid : Colors.grey.shade400, width: 1.8),
            borderRadius: BorderRadius.circular(5)),
          child: value ? const Icon(Icons.check_rounded, color: Colors.white, size: 14) : null),
        const SizedBox(width: 12),
        Text(label, style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w500, color: DAColors.textDark)),
      ]),
    );

  Widget _sectionTitle(String t) => Text(t,
    style: GoogleFonts.poppins(fontSize: 22, fontWeight: FontWeight.w800, color: DAColors.textDark));

  Widget _buildLabel(String t) => Text(t,
    style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w700, color: DAColors.textDark));
}