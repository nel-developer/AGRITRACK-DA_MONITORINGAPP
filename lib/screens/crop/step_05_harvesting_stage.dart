...
class _CropStep5State extends State<CropStep5HarvestingStage> {
  ...
  // Postharvest & processing — file attach + remarks
  String _postharvestFile              = '';
  final List<String> _postharvestRemarks = [];
  String _processingFile               = '';
  final List<String> _processingRemarks  = [];
  ...
  void _next() {
    ...
    w.postharvestFile    = _postharvestFile;
    w.postharvestRemarks = List.from(_postharvestRemarks);
    w.processingFile     = _processingFile;
    w.processingRemarks  = List.from(_processingRemarks);
  ...
  Widget _dateCycleField({
    ...
    GestureDetector(
      ...
      child: Container(
        ...
        child: Row(children: [
          Expanded(child: Text(value.isEmpty ? 'Enter Date Harvested' : value,
    ...
    GestureDetector(
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          const Icon(Icons.attach_file_rounded, color: DAColors.greenMid, size: 22),
        ]),
      ),
    );
    ...
    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      ...list.asMap().entries.map((e) {
        ...
        return Padding(
          ...
          child: Row(children: [
            Expanded(child: _bareField(hint: fieldHint, initial: list[i],
                onChanged: (v) => list[i] = v, kb: kb)),
            if (i > 0) ...[
              const SizedBox(width: 8),
  ...
  Widget _dateCycleList({
    required String sectionLabel, required List<String> list,
  }) =>
    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      _label(sectionLabel),
      const SizedBox(height: 10),
      ...list.asMap().entries.map((e) {
        ...
        return Padding(
          padding: const EdgeInsets.only(bottom: 10),
          child: Row(children: [
            Expanded(child: _dateCycleField(value: list[i],
    ...
    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...list.asMap().entries.map((e) {
        ...
        return Padding(
          ...
          child: Row(children: [
            Expanded(child: _bareField(hint: 'Enter Remark', initial: list[i],
                onChanged: (v) => setState(() => list[i] = v))),
            const SizedBox(width: 8),
            _removeBtn(() => setState(() => list.removeAt(i))),
          ]),
    ...
    return Column(
      ...
      children: [
        ...
        _cycleList(
          ...
          list: _landAreaCycles, addLabel: 'Add Cycle',
          fieldHint: 'Enter land area harvested', kb: TextInputType.number),
        ...
        _cycleList(
          ...
          list: _quantityCycles, addLabel: 'Add Cycle',
          fieldHint: 'Total harvested in kg', kb: TextInputType.number),
        const SizedBox(height: 20),

        CropField(label: 'Average harvest per hectare', hint: 'Enter Average Hectare',
          initialValue: _avgHarvestPerHa, keyboardType: TextInputType.number,
          onChanged: (v) => _avgHarvestPerHa = v),
        const SizedBox(height: 20),

        _cycleList(
          sectionLabel: 'Total cost of harvesting activities per cropping cycle (based on FCA records)',
          list: _harvestCostCycles, addLabel: 'Add Cycle',
          fieldHint: 'Enter the total cost', kb: TextInputType.number),
        const SizedBox(height: 20),

        CropField(label: 'Percentage of harvest goes for food consumption',
          hint: 'Enter percentage', initialValue: _foodConsumptionPct,
          keyboardType: TextInputType.number, onChanged: (v) => _foodConsumptionPct = v),
        const SizedBox(height: 20),

        // ── Postharvest ───────────────────────────────────────────
        _label('Underwent postharvest (Y/N)'),
        const SizedBox(height: 4),
        Text('If Y, accomplish Annex D',
          style: GoogleFonts.poppins(
              fontSize: 12, fontStyle: FontStyle.italic, color: DAColors.textMuted)),
        const SizedBox(height: 8),
        _attachField(value: _postharvestFile,
            onChanged: (v) => setState(() => _postharvestFile = v)),
        const SizedBox(height: 10),
        _remarksList(_postharvestRemarks),
        const SizedBox(height: 20),

        // ── Processing ────────────────────────────────────────────
        _label('Underwent processing (Y/N)'),
        const SizedBox(height: 4),
        Text('If Y, accomplish Annex E',
          style: GoogleFonts.poppins(
              fontSize: 12, fontStyle: FontStyle.italic, color: DAColors.textMuted)),
        const SizedBox(height: 8),
        _attachField(value: _processingFile,
            onChanged: (v) => setState(() => _processingFile = v)),
        const SizedBox(height: 10),
        _remarksList(_processingRemarks),
