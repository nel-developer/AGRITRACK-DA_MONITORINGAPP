...
class _CropStep3State extends State<CropStep3PlantingStage> {
  ...
  static const _machineryOptions = [
    'Animal-drawn', 'Hand tractor', 'Four-wheel tractor', 'Power tiller', 'Others',
  ];
  ...
  Widget _buildForm() {
    return Column(
      ...
      children: [
        ...
        CropField(label: 'Total land area planted', hint: 'Enter Total land area planted',
          initialValue: _totalLandArea, keyboardType: TextInputType.number,
        ...
        CropField(label: 'Others, specify', hint: 'Enter',
        ...
        CropDropdown(
          ...
          hint: 'Choose', value: _usufructAgreement, items: _ynOptions,
        ...
        CropDropdown(
          ...
          hint: 'Choose', value: _machineryType, items: _machineryOptions,
          onChanged: (v) => setState(() {
            _machineryType = v;
        ...
        ..._landPrepCosts.asMap().entries.map((e) {
          ...
          return Padding(
            ...
            child: Row(children: [
              Expanded(child: _bareField(hint: 'Enter Cost', initial: _landPrepCosts[i],
                  onChanged: (v) => _landPrepCosts[i] = v, kb: TextInputType.number)),
              if (i > 0) ...[
                const SizedBox(width: 8),
        ...
        _dateField(label: 'Start date of Land Preparation',
          value: _landPrepStartDate,
          onChanged: (v) => setState(() => _landPrepStartDate = v)),
        const SizedBox(height: 20),

        CropField(label: 'Total number of days for land preparation',
          hint: 'Enter no. of days', initialValue: _landPrepDays,
          keyboardType: TextInputType.number, onChanged: (v) => _landPrepDays = v),
        const SizedBox(height: 20),

        CropField(label: 'Source of water', hint: 'Enter the Source of Water',
          initialValue: _sourceOfWater, onChanged: (v) => _sourceOfWater = v),
        const SizedBox(height: 20),

        _dateField(label: 'Date of planting/transplanting',
          value: _plantingDate,
        ...
        const SizedBox(height: 8),
        Row(children: [
          Expanded(child: _bareField(hint: 'Enter Amount', initial: _seedAmount,
              onChanged: (v) => _seedAmount = v, kb: TextInputType.number)),
          const SizedBox(width: 10),
          Expanded(child: _bareField(hint: 'Enter Unit', initial: _seedUnit,
              onChanged: (v) => _seedUnit = v)),
        ]),
        const SizedBox(height: 20),

        CropField(label: 'Germination rate (based on specifications)',
          hint: 'Enter Rate', initialValue: _germinationRate,
          keyboardType: TextInputType.number, onChanged: (v) => _germinationRate = v),
        const SizedBox(height: 20),

        CropDropdown(label: 'Good germination based on the FCA (Y/N)',
          hint: 'Choose Y/N', value: _goodGermination, items: _ynOptions,
          onChanged: (v) => setState(() {
            _goodGermination = v;
        ...
        CropField(label: 'If no, indicate the probable reason',
          hint: 'Enter the reason', initialValue: _germinationReason,
