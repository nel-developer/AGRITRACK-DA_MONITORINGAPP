...
class CropStep1ProjectBackground extends StatefulWidget {
  const CropStep1ProjectBackground({super.key, required this.controller});
  final CropStepWrapper controller;

  @override
  State<CropStep1ProjectBackground> createState() =>
      _CropStep1ProjectBackgroundState();
}

class _CropStep1ProjectBackgroundState
    extends State<CropStep1ProjectBackground> {

  CropStepWrapper get c => widget.controller;

  // ── FREE-TEXT support interventions (same as livestock) ──────
  ...
  void _next() {
    ...
    c.supportInterventions = _supportInterventions
        ...
        .toList();
    Navigator.of(context).pushNamed(AppRoutes.cropStep2, arguments: c);
  }

  @override
  Widget build(BuildContext context) {
    return CropFormShell(currentStep: 0, onNext: _next, child: _buildForm());
  }
    ...
    return Column(
      ...
      children: [
        ...
        CropDropdown(
          ...
          onChanged: (v) => setState(() {
            c.primaryIntervention = v;
            if (v != 'Others') c.primaryInterventionOther = '';
          }),
        ),
        if (c.primaryIntervention == 'Others') ...[
          ...
          CropField(
            ...
            initialValue: c.primaryInterventionOther,
            onChanged: (v) => c.primaryInterventionOther = v,
          ),
        ],
        const SizedBox(height: 20),

        // ── Support Interventions — free-text, dynamic ─────────
